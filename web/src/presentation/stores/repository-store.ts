...
export const useRepositoryStore = create<RepositoryState>((set, get) => {
  ...
  const refreshTasks = async (state: RepositoryState) => {
    const branchId = state.activeBranchId;
    if (!branchId) return;
    const tasks = await taskRepository.getByBranchId(branchId);
    set({ tasks });
  };
  ...
  return {
    ...
    loadData: async (repositoryId) => {
      ...
      try {
        ...
        set({ repository, branches, tasks, activeBranchId, isLoading: false });
      } catch (err) {
    ...
    mergeBranch: async (sourceId, targetId) => {
      ...
      try {
        ...
        if (state.repository) {
          ...
          set({ branches, tasks, isLoading: false });
        }
      } catch (err) {
    ...
    deleteBranch: async (branchId) => {
      ...
      try {
        ...
        if (state.repository) {
          ...
          const tasks = activeBranchId ? await taskRepository.getByBranchId(activeBranchId) : [];
          set({ branches, tasks, activeBranchId, isLoading: false });
        }
      } catch (err) {
